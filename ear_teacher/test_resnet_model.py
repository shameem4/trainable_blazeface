"""
Test script to verify ResNet-based model initialization and forward pass.

This script tests:
1. ResNet encoder initialization
...
from model import EarVAE, ResNetVAEEncoder
from lightning.module import EarVAELightning


def test_resnet_encoder():
    """Test ResNet encoder initialization and forward pass."""
    print("="*60)
    print("TEST 1: ResNet Encoder")
    print("="*60)

    # Initialize encoder
    print("\n1. Initializing ResNet-50 encoder...")
...
def test_vae_model():
    ...
    # Initialize model
    print("\n1. Initializing ResNet-based VAE...")
    model = EarVAE(
        latent_dim=1024,
        image_size=128,
        resnet_version='resnet50',
...
def test_memory_and_speed():
    ...
    model = EarVAE(latent_dim=1024, image_size=128, resnet_version='resnet50', freeze_layers=2)
    model.eval()
...
def main():
    """Run all tests."""
    print("\n" + "="*60)
    print("RESNET-BASED VAE MODEL TEST SUITE")
    print("="*60)
    print("\nThis will test:")
    print("  1. ResNet encoder initialization and forward pass")
    ...
    try:
        ...
        print("TEST SUMMARY")
        print("="*60)
        print(f"  ResNet Encoder:   {'[PASS]' if test1 else '[FAIL]'}")
        ...
        if all([test1, test2, test3, test4]):
            ...
            print("     rm -rf ear_teacher/checkpoints/* ear_teacher/logs/*")
            print("  2. Start training:")
            print("     cd ear_teacher")
            print("     python train.py --epochs 60 --batch-size 8")
            print("  3. Monitor progress:")
            print("     tensorboard --logdir ear_teacher/logs")
            return 0
        else:

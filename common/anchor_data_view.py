...
class AnchorDataViewer:
    ...
    def __init__(
        ...
        Args:
            ...
            pos_iou_threshold: IoU threshold for positive anchors (default from MATCHING_CONFIG)
        ...
        # Create model to get anchors (BlazeEar generates ear-specific sized anchors)
        self.model = BlazeEar()
        self.anchors = self.model.anchors  # (N, 4) in [cx, cy, w, h] format
        print(f"Total anchors: {len(self.anchors)} (ear-specific sized anchors)")
    ...
    def match_anchors_for_image(self, gt_boxes: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, List[int]]:
        """
        Match anchors to GT boxes using IoU.
        ...
        Returns:
            ...
            best_anchor_per_gt: List of anchor indices with highest IoU per GT
        """
        ...
        # Convert anchors to xyxy format using centralized function
        ...
        # Initialize all as ignore
        matched_labels = torch.full((num_anchors,), -1, dtype=torch.float32)
        
        # Negative: IoU < neg_threshold
        matched_labels[max_iou_per_anchor < self.neg_iou_threshold] = 0
        
        # Positive: IoU >= pos_threshold
        matched_labels[max_iou_per_anchor >= self.pos_iou_threshold] = 1
        
        # Best anchor per GT (highest IoU) - always positive
        best_anchor_per_gt = ious.argmax(dim=0).tolist()  # (M,)
        for anchor_idx in best_anchor_per_gt:
            matched_labels[anchor_idx] = 1
        
        return matched_labels, max_iou_per_anchor, best_anchor_per_gt
    ...
    def draw_anchor_point(self, image: np.ndarray, anchor: torch.Tensor, 
        ...
        For sized anchors, also draws a crosshair at the center.
        """
        h, w = image.shape[:2]
...
Examples:
  ...
  python anchor_data_view.py --pos-iou 0.1 --neg-iou 0.05
    ...
    parser.add_argument('--pos-iou', type=float, default=None,
                       help=f'Positive IoU threshold (default: {MATCHING_CONFIG["pos_iou_threshold"]} from config)')
    parser.add_argument('--neg-iou', type=float, default=None,
                       help=f'Negative IoU threshold (default: {MATCHING_CONFIG["neg_iou_threshold"]} from config)')
